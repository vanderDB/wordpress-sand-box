	...
	jQuery(document).ready(function($) {
		vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
			...
			show: function() {
				...
				$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .sortable_templates ul > li').each(function() {
					"all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template').length) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length)
				}),
				$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"),
				$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .sortable_templates li').on( "click", function() {
					$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .sortable_templates li').removeClass("active"),
					$(this).addClass("active");
					var t = $(this).attr("data-sort");
					$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template').removeClass("hidden"),
					"all" != t && $('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template:not(.' + t + ")").addClass("hidden")
				}),
				$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template', $(this.el) ).removeClass('is-loading').find('.vc-composer-icon').removeClass('vc-c-icon-sync');
				$('.vc_ui-control-button i', $(this.el) ).removeClass('rotating');
				$(this.el).on('click', '.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template [data-template-handler]' ,function() {

					$(this).closest('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template').addClass('is-loading')
		...
		$('.vc_edit-form-tab[data-tab="cs_vc_templates"] .vc_ui-template-list > .vc_ui-template img.lazy').Lazy({
			effect: 'fadeIn',
			delay: 3000,
			bind: "event",
			threshold: 200,
