				...
				wp.media.post(
					...
					function( html ) {
						if ( 1 == settings.post.featuredExisted ) {
							$( '.nmi-current-image-wrapper', '.pgs_menu_popup-content' ).show();
						} else {
							$( '.nmi-upload-link', '.pgs_menu_popup-content' ).hide();
						}
						$( '.nmi-current-image-wrapper', '.pgs_menu_popup-content' ).html( html );
					}
				);
			},
			...
			$.post(
				...
				function(str){
					if ( str == '0' ) {
						...
						$( '.nmi-upload-link', '.pgs_menu_popup-content' ).hide();
						$( '.nmi-current-image', '.pgs_menu_popup-content' ).html( str );
					}
				}
			);
		};
