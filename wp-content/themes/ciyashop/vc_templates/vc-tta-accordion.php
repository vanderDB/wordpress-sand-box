...
class WPBakeryShortCode_VC_Tta_Accordion extends WPBakeryShortCodesContainer {
	 ...
	 * @var string
	 */
	public $non_draggable_class = 'vc-non-draggable-container';
	...
	public function setGlobalTtaInfo() {
		$section_class       = visual_composer()->getShortCode( 'vc_tta_section' )->shortcodeClass();
		$this->section_class = $section_class;

		if ( is_object( $section_class ) ) {
	...
	public function getParamContent( $atts, $content ) {
		$panels_content = wpb_js_remove_wpautop( $content );
		if ( isset( $atts['c_icon'] ) && strlen( $atts['c_icon'] ) > 0 ) {
			$is_page_editable = vc_is_page_editable();
			if ( ! $is_page_editable ) {
				$panels_content = str_replace( '{{{ control-icon }}}', '<i class="vc_tta-controls-icon vc_tta-controls-icon-' . $atts['c_icon'] . '"></i>', $panels_content );
			} else {
				$panels_content = str_replace( '{{{ control-icon }}}', '<i class="vc_tta-controls-icon" data-vc-tta-controls-icon="' . $atts['c_icon'] . '"></i>', $panels_content );
			}
		} else {
			$panels_content = str_replace( '{{{ control-icon }}}', '', $panels_content );
		}
	...
	public function getParamPaginationList( $atts, $content ) {
		...
		$is_page_editabe = vc_is_page_editable();
