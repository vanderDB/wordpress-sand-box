...
do_action('woocommerce_before_add_to_cart_form'); ?>
    <script>
        ...
        function change_option(attribute, target) {
            let select = document.querySelectorAll('.select2-selection__rendered');
            for (const selectKey in select) {
                console.log([selectKey, target]);
            }
                ...
                foreach ($attributes as $attribute_name => $options) {
                    ...
                    <tr>
                        ...
                        <td class="value<?php echo !empty($enable_swatch) ? esc_attr(' ciyashop-swatches') : ''; ?>">
                            ...
                            if ($enable_swatch) {
                                ?>
                                <div class="swatches-select"
                                     data-id="<?php echo esc_attr(sanitize_title($attribute_name)); ?>">
                                    <?php
                                    if (is_array($options)) {
                            ...
                            echo "</div>";
                            woocommerce_form_field($attribute_name, array(
                                'options' => $options,
                                'attribute' => $attribute_name,
                                'product' => $product,
                                'type' => 'radio',
                                'custom_attributes' => ['onclick' => "change_option('{$attribute_name}', this)"]
                            ), '');
