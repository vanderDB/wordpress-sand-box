...
(function ($) {
  ...
  _deprecated = wpColorPickerL10n.current !== undefined; // Declare some global variables when is deprecated or not
  ...
  $.widget('wp.wpColorPicker', $.wp.wpColorPicker, {
    ...
    _create: function _create() {
      ...
      if (_deprecated) {
        ...
        self.toggler.find('.wp-color-result-text').text(wpColorPickerL10n.pick); // Set up the Iris container and insert it after the wrapping label.

        self.pickerContainer = $(_after).insertAfter(self.wrappingLabel); // Store a reference to the Clear/Default button.
